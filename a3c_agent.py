...
class A3CAgent(object):
  ...
  def build_model(self, reuse, dev, ntype):
    with tf.variable_scope(self.name) and tf.device(dev):
      ...
      self.minimap = tf.placeholder(tf.float32, [None, 17, self.msize, self.msize], name='minimap')
      self.screen = tf.placeholder(tf.float32, [None, 23, self.ssize, self.ssize], name='screen')
